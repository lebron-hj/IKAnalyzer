		...
		if(CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()){
			...
			if(!this.countHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					hit = Dictionary.getSingleton(context.getConfiguration().getResourceLoader()).matchWithHit(context.getSegmentBuff(), context.getCursor() , hit);
					if(hit.isMatch()){
						//输出当前的词
			...
			//*********************************
			//对当前指针位置的字符进行单字匹配
			Hit singleCharHit = Dictionary.getSingleton(context.getConfiguration().getResourceLoader()).matchInQuantifierDict(context.getSegmentBuff(), context.getCursor(), 1);
