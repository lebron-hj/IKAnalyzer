...
public class Dictionary{
	 ...
	 * 词典单子实例
	 */
	private static final Map<String,Dictionary> dictionaryMap = new HashMap<String, Dictionary>();
	...
	public static Dictionary getSingleton(ResourceLoader loader){
		...
		if (loader instanceof SolrResourceLoader) {
			...
			String dataDir = "";
			if(srl instanceof ZkSolrResourceLoader){
				dataDir = ((ZkSolrResourceLoader) srl).getDataDir();
			}else{
				dataDir = srl.getDataDir();
			}
			singleton = dictionaryMap.get(dataDir);
		}else{
			singleton = dictionaryMap.get(loader.toString());
		}
...
//	}
	...
	public static void initial(Configuration cfg) {
		if (cfg.getResourceLoader() instanceof SolrResourceLoader) {
			SolrResourceLoader srl = (SolrResourceLoader)cfg.getResourceLoader();
			...
			if(dictionaryMap.containsKey(dataDir)){
				return;
			}
			synchronized(dictionaryMap){
				if (!dictionaryMap.containsKey(dataDir)) {
					//不放一字符串作为key的话，reload就找不到了
					dictionaryMap.put(dataDir, new Dictionary(cfg));
				}
			}
		} else {
			String key = cfg.getResourceLoader()==null?null:cfg.getResourceLoader().toString();
			if(dictionaryMap.containsKey(key)){
				return;
			}
			synchronized(dictionaryMap){
				if (!dictionaryMap.containsKey(key)) {
					dictionaryMap.put(key, new Dictionary(cfg));
				}
			}
		}
	}
