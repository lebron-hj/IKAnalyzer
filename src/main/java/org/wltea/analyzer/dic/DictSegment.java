...
class DictSegment implements Comparable<DictSegment>{
	 ...
	 * @param charArray
	 * @param begin
	 * @param length 字典文件中当前行剩余的文字个数，如果是最后一个，得标识为叶子(ds.nodeState=1)
	 * @param enabled
	 */
...
//		System.out.println(this.toString());
	...
	public String toString() {
		...
		sb.append("##nodeChar:").append(this.nodeChar)
		.append("##storeSize:").append(this.storeSize);
		if (this.storeSize > this.ARRAY_LENGTH_LIMIT) {
			if (this.childrenMap == null ||  this.childrenMap.size() == 0) {
				sb.append("##childrenMap:null");
			} else {
				sb.append("##childrenMap:【");
				for (Iterator<Character> iterator = this.childrenMap.keySet().iterator(); iterator
						.hasNext();) {
			...
			if (this.childrenArray == null ||  this.childrenArray.length == 0) {
				sb.append("##childrenArray:null");
			} else {
				sb.append("##childrenArray:【");
				for (int i=0;i<this.childrenArray.length; i++) {
					if (this.childrenArray[i] == null) {
						sb.append("null,");
					} else {
						Character c = this.childrenArray[i].nodeChar;
						sb.append(c).append(",");
					}
